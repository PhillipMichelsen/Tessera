...
func ConvertGoToProtoMarketDataPacket(goData MarketDataPacket) *generated.MarketDataPacket {
	protoData := &generated.MarketDataPacket{
		...
		UuidHistory: func() []string {
			paths := make([]string, len(goData.UUIDHistory))
			for i, path := range goData.UUIDHistory {
				paths[i] = path.String()
			}
			return paths
		}(),
		UuidHistoryTimestamps: func() []*timestamppb.Timestamp {
			timestamps := make([]*timestamppb.Timestamp, len(goData.UUIDHistoryTimestamps))
			for i, ts := range goData.UUIDHistoryTimestamps {
...
func ConvertProtoToGoMarketDataPacket(protoData *generated.MarketDataPacket) MarketDataPacket {
	goData := MarketDataPacket{
		...
		UUIDHistory: func() []uuid.UUID {
			paths := make([]uuid.UUID, len(protoData.UuidHistory))
			for i, path := range protoData.UuidHistory {
				paths[i], _ = uuid.Parse(path)
			}
			return paths
		}(),
		UUIDHistoryTimestamps: func() []time.Time {
			timestamps := make([]time.Time, len(protoData.UuidHistoryTimestamps))
			for i, ts := range protoData.UuidHistoryTimestamps {
